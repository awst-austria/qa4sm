...
class TestValidation(TestCase):
    ...
    def _check_validation_configuration_consistency(
            self, validation: ValidationRun) -> None:
        """
        ...
        # check if intra-annual metrics are not calculated by default
        if validation.intra_annual_metrics:
            raise ValueError(
                f'Intra-annual metrics are not be calculated by default, but only the bulk case: {validation.intra_annual_metrics=}.'
            )

        # check default values for intra-annual overlap and type
        ...
        elif validation.intra_annual_overlap < 0:
            raise ValueError(
                f'The default intra-annual overlap should not be below 0, but is found to be {validation.intra_annual_overlap=}.'
            )
