...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        mode = input('Do you want to set paths for all datasets (a) or just those that don\'t have one (u)? (default a): ') or 'a'
        mode = 'u' if mode.lower().startswith('u') else 'a'

        parent_data_folder = input('[optional] Set your parent data folder here to auto-generate dataset subfolder suggestions (default: empty): ')
        ...
        for dataset in Dataset.objects.all():
            ...
            if default_path:
                val = input('Please enter storage location for dataset "{}" (default {}): '.format(dataset.short_name, default_path)) or default_path
            else:
