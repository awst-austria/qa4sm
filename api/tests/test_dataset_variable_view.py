...
class TestDatasetVariableView(TestCase):
    ...
    def test_dataset_variable_by_version(self):
        dataset_variable_by_version_url_name ='Dataset_variable_by_version'
        # check if they are properly taken based on a dataset id provided
        response = self.client.get(reverse(dataset_variable_by_version_url_name, kwargs={'version_id': 7}))  # GLDAS GLDAS_NOAH025_3H_2_1
        assert response.status_code == 200
        assert len(response.json()) == 4  # usually there is only 1, but GLDAS has 4

        response = self.client.get(reverse(dataset_variable_by_version_url_name, kwargs={'version_id': 1}))  # C3S_V201706
        assert response.status_code == 200
        assert len(response.json()) == 1

        response = self.client.get(reverse(dataset_variable_by_version_url_name, kwargs={'version_id': 100}))  # wrong id
        assert response.status_code == 404

        # check availability after logging out
        self.client.logout()

        response = self.client.get(reverse(dataset_variable_by_version_url_name, kwargs={'version_id': 7}))  # GLDAS
        assert response.status_code == 200
