...
class TestDatasetVersionView(TestCase):
    ...
    def test_dataset_version(self):
        ...
        response = self.client.get('/api/dataset-version?dataset=1')  # C3S
        assert response.status_code == 200
        assert len(response.json()) == 4  # there are 4 C3S versions currently

        # check wrong dataset id
        response = self.client.get('/api/dataset-version?dataset=100')  # wrong
        assert response.status_code == 404

        # check version id:
        response = self.client.get('/api/dataset-version?version_id=1')
        assert response.status_code == 200
        assert response.json()['short_name'] == 'C3S_V201706'

        response = self.client.get('/api/dataset-version?version_id=1000')  # wrong version id
        assert response.status_code == 404
