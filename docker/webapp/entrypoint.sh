...
if [ "$DB_UPDATE" = "TRUE" ]; then
    ...
    if [ -f "/var/lib/qa4sm-web-val/valentina/$DB_DUMP_NAME" ]; then
        psql -h qa4sm-db -p 5432 -U postgres -d $QA4SM_DB_NAME -q -f "/var/lib/qa4sm-web-val/valentina/$DB_DUMP_NAME"
        NEW_DB="FALSE"
        echo "DB dump uploaded successfully."
    else
        echo "DB dump file not found: /var/lib/qa4sm-web-val/valentina/$DB_DUMP_NAME"
        exit 1
    fi
fi
